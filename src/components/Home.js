...
const Home = () => {
  ...
  if (state === 'Success' && details !== null && arrowBack.current !== null) {
    ...
    const indexOfData = [
      data.rawData.findIndex((obj) => obj.Combined_Key === details),
    ];
    content = (
      <div className="details-row">
        <Component
          ...
          incident={parseFloat(data.rawData[indexOfData].Incident_Rate).toFixed(
            4,
          )}
          cases={data.rawData[indexOfData].Confirmed}
          deaths={data.rawData[indexOfData].Deaths}
          ratio={parseFloat(
            data.rawData[indexOfData].Case_Fatality_Ratio,
          ).toFixed(4)}
        />
      </div>
    );
